...
const Billboard = (p: { billboard: TBillboard }) => {
    return (
        <div className="overflow-hidden rounded-xl p-4 sm:p-6 lg:p-8">
            <div
                style={{ backgroundImage: `url(${p.billboard?.imageUrl})` }}
...
const Billboards = (p: { billboards: TBillboard[] }) => {
    ...
    return (
        ...
        >
            <CarouselContent>
                {p.billboards.map((billboard, index) => (
                    <CarouselItem key={index}>
                        <div className="p-1">
                            <Billboard billboard={billboard} />
                        </div>
                        <div className="flex justify-center gap-1">
                            {Array.from({ length: p.billboards.length }).map((_, i) => (
                                <div
                                    key={i}
